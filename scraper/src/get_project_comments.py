...
class ScrapingConfig(BaseModel):
    ...
    request_delay: float = Field(
        default=0.5, ge=0.1, description="Delay between requests"
    )
...
def setup_logging(config: ScrapingConfig) -> str:
    ...
    log_file = f"logs/logs_scraper_{timestamp}.log"
...
class Cache:
    ...
    def rebuild_from_existing_projects(self) -> None:
        ...
        try:
                ...
                if metadata_file.exists():
                    try:
                        ...
                        if metadata:
                            ...
                            if (
                                project_id
                                and str(project_id) not in self.data["project_ids"]
                            ):
                                self.data["project_ids"].append(str(project_id))
                                logging.debug(
                                    f"Added project ID to cache: {project_id}"
                                )
            ...
            self.save()
            logging.info(
                f"Cache rebuilt: {len(self.data['urls'])} URLs, {len(self.data['project_ids'])} project IDs"
            )
...
def make_api_request(url: str, timeout: float = 30) -> APIResponse:
    ...
    try:
        ...
        return APIResponse(
            False, error=f"HTTP {e.code}: {e.reason}", status_code=e.code
        )
    except Exception as e:
...
def retry_api_request(url: str, config: ScrapingConfig) -> APIResponse:
    ...
    for attempt in range(config.max_retries + 1):
        ...
        if response.status_code == 401:
            logging.error(
                f"401 Unauthorized for {url}. URL signature has likely expired."
            )
            break
        ...
        if attempt < config.max_retries:
            ...
            logging.warning(
                f"Request failed, retrying in {wait_time}s: {response.error}"
            )
    ...
    try:
        ...
        for script in script_tags:
            if script.string and "preload" in script.string:
                match = re.search(
                    r'preload\s*\(\s*".*?project.*?"\s*,\s*({.*?})\)', script.string
                )
                if match:
...
def save_project_files(project: ProjectData, output_dir: str) -> bool:
    ...
    project_dir = Path(output_dir) / (
        project.project_id or create_safe_filename(project.url)
    )
    ...
    metadata = {
        ...
        "data_apis": {
            ...
            "creator": project.data.get("creator", {})
            .get("urls", {})
            .get("api", {})
            .get("user"),
        },
...
def process_api_endpoint(
    url: str, name: str, project_dir: Path, config: ScrapingConfig
) -> bool:
    ...
    if name == "comments":
        ...
        if base_reponse.success:
            ...
            while len(reponse.get("comments", [])) > 0:
                logging.info(
                    f"Found {len(reponse['comments'])} comments in this repose."
                )
                comment_data.append(reponse)
                more_comments_url = (
                    reponse.get("urls", {}).get("api", {}).get("more_comments", {})
                )
        ...
        if base_response.success:
            ...
            while len(response.get("updates", [])) > 0:
                ...
                more_updates_url = (
                    response.get("urls", {}).get("api", {}).get("more_updates", {})
                )
...
def process_project_apis(
    project: ProjectData, output_dir: str, config: ScrapingConfig
) -> bool:
    ...
    if config.scrape_comments and "comments" in api_urls:
        ...
        results.append(
            process_api_endpoint(api_urls["comments"], "comments", project_dir, config)
        )

    if config.scrape_updates and "updates" in api_urls:
        results.append(
            process_api_endpoint(api_urls["updates"], "updates", project_dir, config)
        )
...
def process_single_url(
    url: str, driver: uc.Chrome, cache: Cache, config: ScrapingConfig
) -> bool:
...
def scrape_kickstarter_projects(
    urls: List[str], config: Optional[ScrapingConfig] = None
) -> ScrapingResult:
    """
    ...
    logging.info(
        f"URLs: {len(valid_urls)} total, {len(new_urls)} new, {cached_count} cached"
    )
    ...
    logging.info(
        f"Total: {result.total_urls}, Success: {result.successful}, "
...
def get_projects(
    urls: List[str], base_output_path: str = "projects", cache_file: str = "cache.json"
) -> Dict[str, List[str]]:
    ...
    cache = Cache(config)
    return {"urls": cache.data["urls"], "project_ids": cache.data["project_ids"]}
